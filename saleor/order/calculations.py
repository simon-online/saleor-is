...
def _update_order_discount_for_voucher(order: Order):
    ...
    # TODO: zedzior obczaj case z dwoma voucherami: na shipping in order
    # TODO: ogarnij flow z shipping voucher ?
...
def _calculate_and_add_tax(
    ...
    if tax_calculation_strategy == TaxCalculationStrategy.TAX_APP:
        ...
        # TODO: zedzior propagate order level discount to lines if `tax_data` is empty
    else:
...
def order_line_unit(
    ...
    """
    currency = order.currency
    _, lines = fetch_order_prices_if_expired(order, manager, lines, force_update, info)
...
def order_shipping(
    ...
    """
    currency = order.currency
    order, _ = fetch_order_prices_if_expired(order, manager, lines, force_update, info)
